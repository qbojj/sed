...
type c_cmd = { cmd : c_command; range : c_address_range; neg : bool }
...
let compile (commands : command list) : (compiled_program, string) Result.t =
  let compile_address : address -> c_address = function
    ...
    | Last -> Last
  in
  let compile_address_opt : (address * bool) option -> c_address_range * bool =
    function
    | None -> (Always, false)
    | Some (a, neg) -> (Single (compile_address a), neg)
  in
  let compile_range : address_range -> c_address_range * bool = function
    | Always -> (Always, false)
    | Single (a, neg) -> (Single (compile_address a), neg)
    | Range (start, end_, neg) ->
        ( Range { start = compile_address start; end_ = compile_address end_ },
          neg )
  in
  let compile_replace_info ({ src; dst; flags } : replace_info) : c_replace_info
      =
    let case_insensitive = List.mem CaseInsensitive flags in
    let group_usage_free =
      List.for_all (function Group _ -> false | _ -> true) dst
    in
    let src =
      Regex.compile
        {
          case_insensitive;
          supports_groups = not group_usage_free;
  ...
  let get_pos l =
    match LMap.find_opt l labels with
    | None -> Error ("no label named " ^ l)
    | Some v -> Ok v
  in

  let rec translate (i : int) (acc : c_cmd list) :
      command list -> (c_cmd list * int, string) Result.t = function
    | [] -> return (acc, i)
    ...
    | Replace (range, info) :: tl ->
        let range, neg = compile_range range in
        translate (i + 1)
          ({ cmd = Replace (compile_replace_info info); range; neg } :: acc)
          tl
    | CondJmp (range, l) :: tl ->
        let range, neg = compile_range range in
        let* target = get_pos l in
    ...
    | PrintEscaped range :: tl ->
        let range, neg = compile_range range in
        translate (i + 1) ({ cmd = PrintEscaped; range; neg } :: acc) tl
  in
...
type zipper = { prev : e_cmd list; next : e_cmd list }
...
let restart z = { prev = []; next = List.rev_append z.prev z.next }
...
let get_active st = match st.zipper.next with [] -> None | hd :: _ -> Some hd
...
let e_cmd_is_active st =
  ...
  | Range { start; end_ } ->
      if cmd.is_active then
        ( (if match_address true end_ then
             update_active st { cmd with is_active = false }
           else st),
          true <> cmd.neg )
      else if match_address false start then
        (update_active st { cmd with is_active = true }, true <> cmd.neg)
...
let line_wrap_and_escape wrap (s : string) : string =
  let rec wrap' n s () =
    ...
    | Seq.Cons (hd, tl) ->
        ...
        else (Seq.append escaped (wrap' (n + el) tl)) ()
  in
  s |> String.to_seq |> wrap' 0 |> String.of_seq

let run ({ autoprint; line_wrap } : config) (prog : compiled_program)
    (input : string Seq.t) : string Seq.t =
  ...
  let rec get_line st =
    ...
    | Seq.Cons (hd, tl) ->
        ( Some hd,
          { st with lineno = st.lineno + 1; input = tl; condjmp = false } )
  and new_cycle_with autoprint to_print st =
    let zipper = restart st.zipper in
    if autoprint then Seq.Cons (to_print, run' { st with zipper })
    else run' { st with zipper } ()
  and new_cycle autoprint st =
    let impl () =
      ...
      | None, _ -> Seq.Nil
    in
    if autoprint then Seq.Cons (st.pattern, impl) else impl ()
  and run' (st : state) : string Seq.t =
   fun () ->
    if is_empty st then new_cycle autoprint st
      ...
      if not is_active then run' nst ()
        ...
        match cmd with
        | PrintLineNo -> Seq.Cons (string_of_int lineno, run' nst)
        | Append s -> run' { nst with pattern = pattern ^ s } ()
        | Insert s -> run' { nst with pattern = s } ()
        | QuitAutoprint -> if autoprint then Seq.return pattern () else Seq.Nil
        | Quit -> Seq.Nil
        | AppendFile s ->
        ...
        | ReplaceText s ->
            ...
            close_in file;
            run' { nst with pattern } ()
        | Delete -> new_cycle false nst
        | DeleteLine -> (
            (* delete first line from pattern *)
            (* if there was no new line -> start new cycle *)
            match split_line pattern with
            | _, None -> new_cycle false st
            | _, Some rest ->
                new_cycle_with false "" { nst with pattern = rest })
        ...
        | Next -> (
            match get_line nst with
            | Some pattern, nst -> run' { nst with pattern } ()
            | None, nst ->
                new_cycle autoprint nst (* no line -> autoprint and exit *))
        | AppendNext -> (
            match get_line nst with
            | Some line, nst ->
                run' { nst with pattern = pattern ^ "\n" ^ line } ()
            | None, nst ->
                new_cycle autoprint nst (* no line -> autoprint and exit *))
        ...
        | Transliterate l ->
            let pattern =
              String.map
                (fun c ->
                  match List.assoc_opt c l with None -> c | Some c -> c)
                pattern
            in
            run' { nst with pattern } ()
        | Write s ->
            ...
            close_out file;
            run' nst ()
        | Branch n -> run' { nst with zipper = zmove st.zipper n } ()
        | CondJmp n ->
            if st.condjmp then
              run' { nst with zipper = zmove st.zipper n; condjmp = false } ()
            else run' nst ()
  in
  fun () ->
    ...
    let prog : e_cmd list =
      List.map
        (fun ({ cmd; range; neg } : c_cmd) ->
          { cmd; range; neg; is_active = false })
        prog
    in
    match input () with
    | Seq.Nil -> Seq.Nil
