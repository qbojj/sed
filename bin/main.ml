...
let join_result_seq (input: ('a, string) Result.t Seq.t): ('a Seq.t, string) Result.t =
  input
...
let compile_souces source_list file_list =
  let sources = 
    source_list
    |> List.to_seq
    |> Seq.mapi (fun i s -> Ok ("inline source " ^ string_of_int i, s)) in
  let files =
    file_list
    |> List.to_seq
    |> Seq.map (fun f ->
  ...
  |> Seq.map ((Fun.flip Result.bind) (fun (name, s) ->
      Command.parseScript s
      |> Result.map_error (fun ({ row; column; msg }: Command.parse_error) ->
          "parse error in " ^ name ^ " at " ^ string_of_int row ^ ":" ^ string_of_int column ^ ": " ^ msg
  )))
  |> join_result_seq
...
let get_stream (input_files: string list): (string Seq.t, string) Result.t =
  input_files
    ...
    |> Seq.map (fun f ->
      if Sys.file_exists f then
        Ok (get_file_stream (open_in f) true)
      else
        Error ("file not found: " ^ f)
    )
    |> join_result_seq
...
let run_err source_list file_list no_autoprint line_wrap inplace separate input_files =
  let (let*) = Result.bind in
  let* commands = compile_souces source_list file_list in
  let config: Interpreter.config = { autoprint = not no_autoprint; line_wrap } in
    ...
    if separate then
      ...
      |> Seq.map (fun f ->
        ...
        | Some suffix ->
          ...
          List.iter (fun s -> output_string out_file (s ^ "\n")) output_stream;
          close_out out_file;
          Ok ()
      )
      |> join_result_seq
...
let run source_list file_list no_autoprint line_wrap inplace separate input_files =
  let separate = separate || Option.is_some inplace in
  let (input_files, source_list) =
    match source_list, input_files with
    | [], [] -> ([], [])
    | [], f :: fs -> (fs, [f])
    | _ :: _, _ -> (input_files, source_list) in
  match run_err source_list file_list no_autoprint line_wrap inplace separate input_files with
  | Ok () -> ()
...
let cmd =
  ...
  let source_arg =
    ...
    Arg.(value & opt_all string [] & info ["e"; "expression"] ~docv:"SCRIPT" ~doc) in

  let file_arg =
    let doc = "Append script file to list of source files" in
    Arg.(value & opt_all string [] & info ["f"; "file"] ~docv:"FILE" ~doc) in

  let no_autoprint_arg =
    let doc = "Suppress automatic printing of pattern space" in
    Arg.(value & flag & info ["q"; "quiet"; "silent"] ~doc) in

  let line_wrap_arg =
    let doc = "Specify line wrap length" in
    Arg.(value & opt int Int.max_int & info ["l"; "line-length"] ~docv:"N" ~doc) in

  let inplace_arg =
    let doc = "Edit files in place (optional suffix)" in
    Arg.(value & opt ?vopt:(Some (Some "")) (some string) None & info ["i"; "in-place"] ~docv:"SUFFIX" ~doc) in

  let separate_arg =
    let doc = "Consider files separately rather than as a single continuous stream" in
    Arg.(value & flag & info ["s"; "separate"] ~doc) in
  
  let files_arg =
    let doc = "Files to process" in
    Arg.(value & pos_all string [] & info [] ~docv:"FILE" ~doc) in

  Cmd.v info Term.(const run $ source_arg $ file_arg $ no_autoprint_arg $ line_wrap_arg $ inplace_arg $ separate_arg $ files_arg)
